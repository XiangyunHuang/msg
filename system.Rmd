...
### plotly with ggplot {#subsec:plotly-ggplotly}

如果你对plotly不熟悉，但是你对ggplot2比较熟悉，那么恭喜你：plotly包中，函数 `ggplotly` 可以直接将 ggplot2 对象转化为 plotly 对象。这意味着你不需要学习plotly相关的技能，即可通过简单的一个函数，基于ggplot2制作交互图形。
...
gg <- ggplotly(g)
```
```{r faithful,fig.cap="交互式图形",out.extra = ifelse(is_latex,'','data-url="interactives/faithful.html"'),echo=FALSE}
knitr::include_graphics(path = "interactives/faithful.svg")
```
...
```
```{r plotly-histogram,echo=FALSE,fig.cap="直方图",out.extra = ifelse(is_latex,'','data-url="interactives/plotly-histogram.html"')}
...
knitr::include_graphics(path = "interactives/plotly-histogram.svg")
```
...
```
```{r plotly-heatmap-contour,fig.cap="交互式密度图",out.extra = ifelse(is_latex,'','data-url="interactives/plotly-heatmap-contour.html"'),echo=FALSE}
...
knitr::include_graphics(path = "interactives/plotly-heatmap-contour.svg")
```
...
```
```{r heatmaply,fig.cap="交互式热图",out.extra = ifelse(is_latex,'','data-url="interactives/heatmaply.html"'),echo=FALSE}
knitr::include_graphics(path = "interactives/heatmaply.svg")
```
...
```
```{r plotly-china-map,fig.cap="交互式地图",out.extra = ifelse(is_latex,'','data-url="interactives/plotly-china-map.html"'),echo=FALSE}
...
knitr::include_graphics(path = "interactives/plotly-china-map.svg")
```
...
```
```{r rgl-beta,fig.scap="(ref:rgl-beta-s)",fig.cap="(ref:rgl-beta)",out.extra = ifelse(is_latex,'','data-url="interactives/rgl-beta.html"'),echo=FALSE}
...
从版本 2.6 开始，**animation** 包已经收录了大约 30 个统计学主题相关的函数，函数主题包括布朗运动、布丰投针、大数定律、中心极限定理、牛顿法求根、二分法求根、梯度下降法、抛硬币、置信区间、交叉验证、K-Means 聚类、k 近邻方法、最小二乘法、蒙特卡罗、豌豆机和抽样方法等。感兴趣的读者可以访问网站 <https://yihui.name/animation/examples/> 看在线演示。
...
1. **fun** 包 [@fun] 中有一个 3D 版本的“我的中国心”，效果如图 \@ref(fig:China-Heart-3D)；请对照 `demo('ChinaHeart3D', package = 'fun')` 以及代码中的源链接思考它的制作过程，并学习如何将平面图形嵌入 rgl 三维图形。
    ...
    ```{r China-Heart-3D,fig.cap="(ref:China-Heart-3D)",echo=FALSE,out.extra = ifelse(is_latex,'','data-url="interactives/rgl-china-heart.html"')}
