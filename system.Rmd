...
[^trellis]: 原网页 http://cm.bell-labs.com/cm/ms/departments/sia/project/trellis/ 已失效，[快照](https://web.archive.org/web/20081019072358/http://cm.bell-labs.com/cm/ms/departments/sia/project/trellis/)，原始网页已无法访问。
...
## 交互式图形 {#sec:interactive-graphics}

自 htmlwidgets [@htmlwidgets] 出世以来，涌现出越来越多的 R 包具备绘制交互式图形的能力，比如 plotly [@plotly]
...
Orca (Open-source Report Creator App) 软件针对 plotly.js 库渲染的图形具有很强的导出功能， [安装 Orca](https://github.com/plotly/orca#installation) 后，`plotly::orca` 函数可以将 plotly 图形对象导出为高质量静态图片，支持导出 png、pdf 和 svg 等格式。
...
```
```{r animation-pollen,fig.scap="pollen 数据中暗藏的特征",fig.cap="(ref:animation-pollen)",echo=FALSE,fig.subcap=c("",""),out.width="45%"}
knitr::include_graphics(path = c(
