...
除了基础图形系统之外，R 还自带另一套图形系统即 **grid**（网格图形）。网格图形系统是一套基础设施性质的图形系统，它本身不包含统计图形，只是提供了一些画图形元素的工具。在此基础上诞生了 **lattice** 图形系统和 **ggplot2** 图形系统，其中 **lattice** 包已经随 R 本身发布，**ggplot2** 包目前的地位还只是一个附加包，但由于它的灵活和美观，用户数量与日俱增，所以我们在本章优先介绍它，并且建议读者可以不必学习 **lattice** 系统。
...
```{r lattice-iris-hist,echo=TRUE, fig.cap="(ref:lattice-iris-hist)",fig.scap="(ref:lattice-iris-hist-s)"}
...
```{r lattice-data-par,echo=TRUE, fig.scap="(ref:lattice-data-par-s)",fig.cap="(ref:lattice-data-par)",fig.subcap=c("",""),fig.ncol=1,fig.width=4.8,fig.height=2.2}
...
```{r, animation.hook="gifski", dev='png', eval=FALSE}
for (i in 1:2) {
  pie(c(i %% 2, 6), col = c("red", "yellow"), labels = NA)
}
```

上面那个例子是用 R 基础函数生成基本图形，下面这个例子将使用 **ggplot2** 生成的图形，并结合 gganimate 制作动画 <https://github.com/thomasp85/gganimate>

```{r mtcars-animate, fig.show='animate', message=FALSE, interval=1/10, eval=is_latex, echo=TRUE}
library(ggplot2)
library(gganimate)
