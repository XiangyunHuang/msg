...
pkgs <- c("Cairo", "formatR", 
          "ggplot2", "MSG", "grid", "magrittr", "ncovr", "kableExtra", "knitr", "lattice", "patchwork", "RColorBrewer", "qqplotr")
...
library(MSG)
msg_code <- function(x, filter = 0) {
  cat("```r\n")
  msg(fig = x, show_code = TRUE, print_plot = FALSE, filter = filter)
  cat("```\n")
}

msg_graph <- function(x) {
  msg(fig = x, show_code = FALSE, print_plot = TRUE)
}
