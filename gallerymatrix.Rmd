...
矩阵数据在形式上属于二维数据（行$\times$列），实际上有三个变量，即每个网格的数值、该网格对应的行数和列数。由于其形式的特殊，我们在本章专门介绍这类数据的统计图形。
...
kable2(ChinaLifeEdu, "我国人口预期寿命和高学历人口数量", insert_txt = "（实际数据来自 2000 年）")
```
...
\noindent 这是在已知二维核密度值的情况下的用法。实际上，**ggplot2** 包的 `geom_density2d()` 函数更方便地绘制等高线，无需事先计算二维核密度：
...
```{r tab-volcano}
data(volcano)
kable2(volcano, "新西兰 Maunga Whau 火山高度矩阵数据")
```
...
```{r image24,fig.width=4,fig.height=1.2,out.width="75%", fig.cap="(ref:fig-mage24)",fig.scap="(ref:fig-mage24-s)"}
# par(mar = rep(0, 4))
...
ggplot(transform(reshape2::melt(volcano), 
                 x = Var1 * 10, y = Var2 * 10),
...
相比其二维平面图形来说，三维透视图（Perspective Plot）在视觉上更具有吸引力。三维透视图的数据基础是网格数据（参考 \@ref(sec:contour) 小节和图 \@ref(fig:contour-grid)），它将一个矩阵中包含的高度数值用曲面连接起来，便形成了我们所看到的三维透视图。透视图可以参见图 \@ref(fig:persp-pop)。
...
```{r matplot,echo=TRUE,fig.width=4.8,fig.height=3,out.width="80%", fig.cap="(ref:fig-matplot)",fig.scap="(ref:fig-matplot-s)"}
...
```{r matplot2,fig.width=4.8,fig.height=3,out.width="80%", fig.cap="(ref:fig-matplot2)",echo=TRUE}
...
```{r heatmap,fig.width=6,fig.height=6,fig.cap="(ref:fig-heatmap)",fig.scap="(ref:fig-heatmap-s)"}
# 用极端化调色板
