...
```{r dotchart,fig.width=4.8,fig.height=4,fig.cap="(ref:fig-dotchart)",fig.scap="(ref:fig-dotchart-s)"}
msg_graph("dotchart")
```
...
```{r dotchart2, fig.width=4.8,fig.height=4,fig.cap="(ref:fig-dotchart2)",fig.scap="(ref:fig-dotchart2-s)"}
msg_graph("dotchart2")
```
...
图 \@ref(fig:rug) 为坐标轴须的一个示例。在 \@ref(sec:hist) 小节中我们使用喷泉数据制作了直方图和密度图以便查看间隔时间的分布，这里我们以喷泉喷发时间为例，在密度曲线的基础上添加上坐标轴须，以使我们能更清楚了解喷发时间数据的具体位置，而不是仅仅看一条曲线。从坐标轴须的疏密我们也可以知道分布密度的大小，这与密度曲线是相辅相成的。

(ref:fig-rug-s) 带坐标轴须的喷泉喷发时间密度曲线图

(ref:fig-rug) 带坐标轴须的喷泉喷发时间密度曲线图
...
```{r stripchart2, fig.width=4.8,fig.height=2.4,out.width="80%", fig.cap="(ref:fig-stripchart2)"}
