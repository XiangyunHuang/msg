...
茎叶图（Stem-and-Leaf Plot）与直方图的功能类似，也是展示数据密度的一种工具。相比之下，茎叶图对密度的刻画显得非常粗略，而且对原始数据通常会作舍入处理，它只是在早期计算机尚不发达时对于手工整理数据来说比较方便。
...
我们分别从正态分布 $\mathrm{N}(0,1)$ 和 $\mathrm{N}(0.5,1)$ 中各自产生 150 和 50 个随机数，然后作箱线图比较两组数据中间位置的差异。用下面的代码得到 图 \@ref(fig:rnorm-boxplot)：
...
这里以1940 年弗吉尼亚州分年龄组、分地区和分性别死亡率数据（表 \@ref(tab:vadeaths-data)）为例，介绍以下条形图。
...
我们可以用下面的代码绘制弗吉尼亚死亡数据条形图，展示了参数 `beside` 和 `legend.text` 的效果：
...
**ggplot2** 包里虽然没有函数直接绘制点图，然而可以借助散点图完成，代码如下：

(ref:fig-dotchart2-s) 用 **ggplot2** 绘制的弗吉尼亚死亡率点图

(ref:fig-dotchart2) 用 **ggplot2** 绘制的弗吉尼亚死亡率点图

```{r dotchart2, echo=TRUE,fig.width=4.8,fig.height=2.7,fig.cap="(ref:fig-dotchart2)",fig.scap="(ref:fig-dotchart-s2)"}
...
严格来说，坐标轴须不能算作是图形，它只是图形的附加物，从 R 基础作图函数 的角度来说，它只是低层作图函数，但由于它在表达数据上的优势，这里我们也将其列入本书的图库中。
...
下面的代码给出了图 \@ref(fig:pie-chart) 里饼图的作法：
...
正如前文所说，这并非一种展示数据的好方式。比它更糟糕的，是 三维饼图。在 R 语言里，可以用 **plotrix** 包的 `pie3D()`函数绘制；
...
图 \@ref(fig:qqnorm) 左图是喷泉间隔时间数据的正态分布 QQ 图（\@ref(sec:hist) 小节的直方图用到过），注意其中的数据经过了标准化，使之均值为 0，方差为 1。
...
1.  在第 \@ref(sec:qqplot) 节中我们了解了如何画 QQ 图，与之对应的还有一种图形叫 PP 图（Probability-Probability），它也是一种检验数据分布是否和理论分布吻合的图形工具，原理和 QQ 图类似：对数据的实际概率分布值和理论概率分布值作散点图即可（也可以选择性地添加一条直线）。编写函数画出 `geyser$waiting` 的 PP 图（理论分布选择正态分布，均值和标准差用矩估计获得），并评价该数据的正态性[^cos18521]。
