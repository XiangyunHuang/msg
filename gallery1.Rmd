...
如同 \@ref(sec:begin) 节中曾经提到的，条形图目前在各种统计图形中应用最广泛，但条形图所能展示的统计量比较贫乏：它只能以矩形条的长度展示原始数值，对数据没有任何概括或推断。
...
回顾前面 \@ref(sec:rug) 小节中我们曾经用坐标轴须在坐标轴上标记出原始数据，这与带状图在一条直线上用点表达原始数据的想法有异曲同工之妙。然而，坐标轴须本身没有堆砌和随机打乱数据的功能（可以通过 `jitter()` 函数实现）。所以，有时候不妨视情况向图中添加带状图作为变量分布的辅助性描述。
