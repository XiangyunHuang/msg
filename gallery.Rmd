...
```{r contour-pop,fig.width=4,fig.height=3,results="hide",fig.cap="(ref:fig-contour-pop)",fig.scap="(ref:fig-contour-pop-s)"}
...
```{r mage24,fig.width=4,fig.height=1.2,fig.cap="(ref:fig-mage24)",fig.scap="(ref:fig-mage24-s)"}
par(mar = rep(0, 4))
...
pairs(iris[1:4],
  upper.panel = function(x,
                           y, ...) points(x, y, pch = c(17, 16, 6)[idx], col = idx), pch = 20, oma = c(
    2,
    2, 2, 2
  ),
...
```{r smoothScatter,fig.width=5,fig.height=5,tidy =FALSE,fig.cap="(ref:fig-smoothScatter)",fig.scap="(ref:fig-smoothScatter-s)"}
...
```{r symbols-pop,fig.width=4.8,fig.height=5,results="hide",fig.cap="(ref:fig-symbols-pop)",fig.scap="(ref:fig-symbols-pop-s)"}
...
```{r survival,fig.width=4.8,fig.height=4.8,fig.cap="(ref:fig-survival)",fig.scap="(ref:fig-survival-s)"}
library(survival)
...
```{r rpart,fig.width=4,fig.height=2.3,fig.cap="(ref:fig-rpart)",fig.scap="(ref:fig-rpart-s)"}
...
```{r map,fig.width=6,fig.height=4,fig.cap="(ref:fig-map)",fig.scap="(ref:fig-map-s)",error=TRUE}
...
```{r faces2,fig.width=4.8,fig.height=5,fig.cap="(ref:fig-faces2)",fig.scap="(ref:fig-faces2-s)"}
library(TeachingDemos)
...
在 R 中有很多包都可以画平行坐标图，例如 **GGally** 包 [@GGally] 中的 *ggparcoord()* 函数、**MASS** 包 [@Venables02] 中的 *parcoord()* 函数和 **iplots** 包 [@iplots] 中的 *ipcp()* 函数等。由于后面 \@ref(ggplot2) 小节还会专门介绍 **ggplot2**，这里我们先看看这个包中的平行坐标图。 *ggparcoord()* 的用法如下：
