...
curve(sin(x) / x, from = -20, to = 20, n = 200, 
      ...
      ylab = latex2exp::TeX("$\\varphi_{X}(t)$"))
```
...
tbl <- plyr::ddply(esoph, agegp ~ alcgp, summarise, mean(ncases / (ncases + ncontrols)))
reshape::cast(tbl, agegp ~ alcgp)
```
...
```{r message=FALSE}
library(vioplot, warn.conflicts = FALSE)
usage(vioplot)
```
