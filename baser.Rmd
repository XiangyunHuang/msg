...
R 语言绘制的统计图形都是通过相应的图形函数生成的。其中，很多图形函数都包含了默认的图形细节设置，这些细节对不太苛刻的用户来说大致可以满足需要，但是往往由于其它方面的要求（如排版、强调某一部分），我们可能要对图形作一些细节性的微调，比如字体、字号、图形边距、点线样式等等。本章介绍 R 语言基础图形系统中的细节参数设置。
...
```{r par1,fig.height= 7, fig.width=7,  fig.cap="(ref:par1)",fig.scap="(ref:par1-s)",echo=FALSE}
...
```{r par2,fig.height=7, fig.width=7, fig.cap="(ref:par2)",fig.scap="(ref:par2-s)",echo=FALSE}
...
```{r plot-region,fig.width=7, fig.height=3.5, echo=FALSE,fig.cap="(ref:plot-region)",fig.scap="(ref:plot-region-s)"}
...
```{r ColorBrewer-all,fig.width=6, fig.height= 8, fig.scap="(ref:ColorBrewer-all-s)",fig.cap="(ref:ColorBrewer-all)"}
...
```{r point-type, fig.width=7, fig.height=3.5, out.width="80%", results='hide',fig.scap="(ref:point-type-s)",fig.cap="(ref:point-type)"}
...
```{r point-iris, out.width="80%", echo=TRUE,fig.scap="(ref:point-iris-s)",fig.cap="(ref:point-iris)"}
...
关于曲线、直线和线段函数的示例代码和效果参见图 \@ref(fig:lines-demo)。
...
类似于线段，前四个参数表示箭头的起点和终点坐标。其他参数简介如下：
...
令人意外的是，跟绘制箭头相比，这个函数更多情况下是用来绘制误差线的：只需将箭头尖短线的角度设置为 $90\,^{\circ}$ 即可。下面的代码将绘制一个带有误差线的条形图（见图 \@ref(fig:error-bars)）：
...
关于误差线的讨论，见 \@ref(sec:stat-principle) 节。
...
kable2(x = Export.USCN, "1999~2004 年中美两国的出口贸易总额数据", only_head = FALSE)
```
...
```{r axis-demo, echo=TRUE,fig.width=7, fig.height=4, out.width="80%", fig.scap="(ref:axis-demo-s)",fig.cap="(ref:axis-demo)",dev='tikz',fig.process=to_png}
data(Export.USCN, package = "MSG")
...
```{r layout,fig.width=7, fig.height=2, out.width="80%", fig.cap="(ref:layout)"}
...
```{r waterfall-chart,echo=TRUE, fig.width=7, fig.height=5, out.width="80%", fig.scap="(ref:waterfall-chart-s)",fig.cap="(ref:waterfall-chart)"}
par(mar = c(4, 4, .5, .1))
...
8. 图 \@ref(fig:point-iris) 在点的样式选取上是否有明显的缺陷？或者说散点图中的点应该如何选取形状以使得各组点之间的外观差异尽量大？第 \@ref(subsec:clear-symbols) 小节会对这个问题有详细评论。

9. 当箭头角度参数 `angle` 为 $90\,^{\circ}$、`code` 参数为 3 时，箭头的形状是怎样的？这种形状是否和数学上的“区间”有类似之处？请考虑如何利用这种“双箭头”表达统计学中的置信区间。

10. 利用三角形的边长关系画一个指北箭头如下

```{r north-arrow,out.width="30%",echo=FALSE}
...
11. 一些数学上函数可以产生有趣的“心形曲线”，例如在极坐标系下 $r(\theta)=1-\sin(\theta)$ 生成的就是心形的曲线，笛卡尔坐标下方程 $(x^{2}+y^{2}-1)^{3}-x^{2}y^{3}=0$ 对应的解也可以形成心形曲线。这里我们考虑一种简单的心形曲线如下图所示：

```{r heart-curve,out.width="30%",echo=FALSE}
...
```{r thermometer-demo,fig.width=4,fig.height=1.5,out.width="75%",echo=FALSE}
par(mar = rep(0.5, 4))
plot.new()
for (i in c(.2, .5, .8)) {
