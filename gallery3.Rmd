...
(ref:fig-worldmap-s) 世界地图

(ref:fig-worldmap) 世界地图：上图是平面地图，下图是以北纬 20° 东经 90° 视角看到的球状地图

```{r worldmap,fig.width=6,fig.height=8,fig.cap="(ref:fig-worldmap)",fig.scap="(ref:fig-worldmap-s)",error=TRUE,results='hide', echo=TRUE, fig.show='hold'}
m0 <- ggplot() + 
...
除了 **maps** 包和 **ggplot2** 包之外，R 语言还有更多的地图绘制系统，例如 **RgoogleMaps** 包 [@RgoogleMaps]，将 Google Maps 提供的（卫星）地图数据引入 R 中，这里简单介绍以下。
...
2. **ggplot2** 并没有现成的函数来绘制条件分割图。不过，下面的代码可以做出类似的效果，如图 \@ref(fig:coplot2)。比较一下，这副图与图 \@ref(fig:coplot) 的区别在哪里？能得出相同的结论吗？如何更改这段代码，以做出图 \@ref(fig:coplot)？
