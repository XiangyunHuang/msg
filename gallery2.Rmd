...
kable2(head2(BinormCircle), "一组人造的散点图数据（部分）")
```
...
```{r tab-aml}
library(survival)
kable2(head2(aml), "急性髓细胞白血病病人生存数据（部分）")
```
...
kable2(head2(fail_temperature), "美国国家航空和宇宙航行局的一批 O 型环失效数据")
```
...
kable2(head2(car.test.frame), "汽车测试报告数据（部分）")
```
