...
kable2(tables$pork, "2006 到 2011 年 猪肉价格")
```
...
```{r tab-canabalt}
data(canabalt)
kable2(head(canabalt), "“末日狂奔”游戏得分消息（前 6 行）")
```
...
```{r tab-music}
data(music)
kable2(head(music[, 1:5]), "音乐曲目数据（前 6 行）")
```
...
```{r music-3d-pcp,fig.cap="(ref:music-3d-pcp)",fig.scap="(ref:music-3d-pcp-s)",message=FALSE,fig.subcap=c("",""),fig.ncol=1,fig.width=4.8,fig.height=2.8}
...
```{r tab-tvearn}
data(tvearn)
kable2(head(tvearn[order(tvearn$pay, decreasing = TRUE), 1:4], 10), "电视剧演员薪酬（前十名）")
```
...
```{r tvearn,fig.cap="(ref:tvearn)",fig.scap="(ref:tvearn-s)",fig.subcap=c("",""),fig.ncol=1}
...
data(CLELAL09, package = "animation")
 # 前 10 行数据
kable2(head(CLELAL09, 10), "一场篮球比赛的球员坐标（部分）")
```
...
kable2(head(assists[,1:6]), "一场篮球比赛的助攻数据（部分）")
```
...
kable2(head(gov.cn.pct), "中国政府网站中的百分比数据（部分）")
```
...
这批数据收录在 **MSG** 包中，可以用以下代码加载，并绘制图 \@ref(fig:gov-cn-pct)：
...
kable2(head(PlantCounts), "植物物种数目与海拔高度（部分）")
```
...
kable2(head(SongWords[, 1:8]), "宋词词频矩阵（部分）")
```
...
```{r multireg-scatter,echo=TRUE, fig.cap="(ref:multireg-scatter)",fig.scap="(ref:multireg-scatter-s)",fig.width=7,fig.height=3.5}
