...
最近几年我们都比较关心物价问题，民间也有不少描述物价上涨的方式，比如作者曾经在邮件中收到一幅"猪肉价格走势图"（图 \@ref(fig:pork-price-orig)）。
...
2020年春节前后，新冠疫情爆发，亿万人的生活和工作都或多或少受到疫情的影响。在疫情刚刚开始的元月份，我们就开始关注疫情的发展情况。那段日子里，笔者每天都在追踪新的疫情报道。有一天，朋友分享了一幅地图（如图 \@ref(fig:ncov-jiangsu)）。地图显示了 1 月 22 日全国各省确诊和疑似病例，只见沿海地区一片火红（红色表示新冠肺炎确诊病例），唯有江苏省是白色。图上搭配文字：“在吗，救我”，在地图上格外抢眼。
...
其实，我们可以看到，在图 \@ref(fig:ncov-jiangsu) 下方给出的数字中，很多省份其实只有 1 例确诊，却涂上了跟几百例确诊的省份同样的颜色，夸大了江苏和其他省份的差别。当然，笔者作图的角度是为了比较各省确诊病例的数量级；如果是为了区分“零确诊”与“有确诊”，那就是另外一回事了。在这个案例里，颜色的选择和图例的重要性可见一斑。

为了直观地看到疫情在地理上的变化，笔者将每天各省的确诊病例做成一张地图，然后连起来做成一幅动画，可以在线观看[^map_animation]。关于笔者参与的有关新冠疫情的进一步研究工作，参见 @zou2020。
...
```{r music-andrews,fig.scap="(ref:music-andrews-s)",fig.cap="(ref:music-andrews)",fig.width=4.9,fig.height=2.8,dev='tikz',fig.process=to_png,fig.showtext=FALSE}
...
```{r cn-us-guardian,fig.cap="(ref:cn-us-guardian)",fig.scap="(ref:cn-us-guardian-s)",echo=FALSE,out.width="70%",fig.link="https://www.guardian.co.uk/news/datablog/2011/jan/19/china-social-media"}
...
```{r cn-us-redraw,fig.cap="(ref:cn-us-redraw)",fig.scap="(ref:cn-us-redraw-s)"}
...
```{r tvearn,fig.cap="(ref:tvearn)",fig.scap="(ref:tvearn-s)",fig.subcap=c("",""),fig.ncol=1, out.width="90%"}
...
```{r rchive-ts, out.width="33%", fig.cap="(ref:rchive-ts)", fig.scap="(ref:rchive-ts-s)", fig.show='hold'}
